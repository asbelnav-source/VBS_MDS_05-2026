...
(Figure 3 Panel B / Figure 4 Panel B depending on final layout). The analysis
tests whether Vascular Burden Score components contribute classifier-relevant
information beyond candidate nigral diffusion microstructural biomarkers in
the subset of PPMI participants with baseline DTI data.
...
- Features:      Six total features in the full model:
                   ...
                   - Nigral L1     (largest eigenvalue, axial diffusivity)
                   - Nigral L2     (middle eigenvalue, radial component 1)
                   - Nigral L3     (smallest eigenvalue, radial component 2)
...
AUC point estimates are sensitive to seed and fold assignment. Running this
script with the default RANDOM_SEED = 42 produces AUC values within
approximately +/- 0.05 of those reported in the manuscript text, depending
on the specific scikit-learn version. The qualitative pattern (DTI features
dominating permutation importance, VBS components contributing measurable
additional importance, modest overall classification) is robust across
seeds. Setting a different RANDOM_SEED yields slightly different point
estimates but consistent feature-importance ordering.
...
# -----------------------------------------------------------------------------
# DTI data prep: aggregate the 6 substantia nigra ROIs per participant
# -----------------------------------------------------------------------------
def build_dti_features(dti_long):
    """Reshape long-format PPMI DTI ROI file into per-participant features.

    PPMI's DTI file has one row per participant per measure (E1, E2, E3, FA)
    with six SN ROIs as columns. We average across the six anatomical
    sub-regions of substantia nigra for each measure to yield a single
    per-participant value.

    Returns DataFrame with columns: PATNO, FA, L1, L2, L3
    """
    sn = dti_long[dti_long["Tissue"] == "SN"].copy()
    sn["roi_mean"] = sn[["ROI1", "ROI2", "ROI3", "ROI4", "ROI5", "ROI6"]].mean(axis=1)
    # Pivot: rows are PATNO, columns are E1/E2/E3/FA
    wide = sn.pivot_table(index="PATNO", columns="Measure", values="roi_mean", aggfunc="mean")
    wide = wide.rename(columns={"E1": "L1", "E2": "L2", "E3": "L3"})
    # Keep only the four measures we need
    return wide[["FA", "L1", "L2", "L3"]].reset_index()
...
def fit_and_evaluate(X, y, feature_set_label, seed=RANDOM_SEED):
    ...
    # Cross-validated probability predictions for AUC
    ...
    # Fit a single model on full data for permutation importance
    rf.fit(X, y)
...
# -----------------------------------------------------------------------------
def main():
    # 1. Load analytic file
    analytic = pd.read_excel(ANALYTIC_FILE)
    print(f"Loaded analytic file: N = {len(analytic)}")

    # 2. Load and reshape DTI
    dti_long = pd.read_csv(DTI_FILE)
    dti = build_dti_features(dti_long)
    print(f"DTI-phenotyped participants: N = {len(dti)}")

    # 3. Merge analytic with DTI; restrict to HC and clinical PD
    df = analytic.merge(dti, on="PATNO", how="inner")
    df = df[df["Cohort_n"].isin([0, 2])].copy()  # 0 = HC, 2 = PD
    ...
    feature_sets = {
        "full": ["VBS_micro", "VBS_macro", "FA", "L1", "L2", "L3"],
        "dti_only": ["FA", "L1", "L2", "L3"],
    ...
    for label, features in feature_sets.items():
        ...
        for feat, imp in sorted(result["permutation_importance"].items(),
                                  key=lambda x: -x[1]):
            print(f"    {feat:<12}: {imp:+.4f}")
        print()

    # 6. Save flat results table
    flat = []
    for r in results:
